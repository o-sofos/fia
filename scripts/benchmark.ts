...
// Configuration
const FIA_BENCH_DIR = path.resolve("benchmark_repo/frameworks/keyed/fia");
...
async function main() {
  try {
    // 1. Build Fia Benchmark
    console.log("🏗️  Building Fia Benchmark...");
    await runCommand("bun", ["run", "build_benchmark.ts"], FIA_BENCH_DIR);
    ...
    // Find result files
    // Filename format: fia-v0.2.9-keyed_01_run1k.json
    const files = fs
      .readdirSync(RESULTS_DIR)
      .filter((f) => f.startsWith("fia") && f.endsWith(".json"));
    ...
    for (const file of files) {
      ...
      // Format: fia-v..._BENCHMARK_ID.json ... wait, benchmark id is at the end?
    ...
    console.log(
      `| ${"Benchmark".padEnd(20)} | ${"Fia".padEnd(10)} | ${"SolidJS".padEnd(10)} | ${"Vue 3".padEnd(10)} | ${"React".padEnd(10)} |`,
    );
    console.log(
      "--------------------------------------------------------------------------------",
    );
    ...
    for (const [id, baseline] of Object.entries(BASELINES)) {
      const fiaVal = results[id];
      const fiaDisplay = fiaVal ? `${fiaVal.toFixed(1)} ms` : "N/A";

      console.log(
        `| ${baseline.label.padEnd(20)} ` +
          `| ${fiaDisplay.padEnd(10)} ` +
