...
# Node.js
npx jsr add @fia/core`)}),G("Quick Start",()=>{q("Create your first reactive app in seconds."),Z(`import { $, div, h1, button, p } from "fia";
...
div({ class: "app" }, () => {
  ...
  button({ textContent: "-", onclick: () => state.count-- });
});`)}),G("Mounting",()=>{q("For Single Page Apps (SPAs), use the mount helper to attach to a root element."),Z(`import { mount, div } from "fia";

const App = () => div(() => {
  // Your app structure
});

mount(App, "#app"); // Clears #app and mounts App`)})}),$("Element API","element-api",()=>{q("Fia elements have a simple, consistent API. Functions match HTML tag names."),Z(`el()                      // Empty element
el({ props })             // Props only
el(() => { ... })         // Children only
...
});`)})}),$("Reactivity","reactivity",()=>{G("Signals",()=>{q("Signals are the primitive units of reactivity."),Z(`const count = $(0);
...
});`)})}),$("Component Composition","components",()=>{q("In Fia, components are just functions. There is no special class or type."),G("Basic Component",()=>{Z(`function Button(props: { text: string }) {
  return button({
...
ul(() => items.forEach(item => li({ textContent: item })));`)})}),G("\uD83D\uDFE1 Intermediate",()=>{D("6. Reactive Store Counter",()=>{q("Objects passed to $() become reactive stores."),Z(`const state = $({ count: 0 }, "count");
...
});`)})}),G("\uD83D\uDD34 Advanced",()=>{D("11. Todo App",()=>{q("A complete todo app using Each."),Z(`const todos = $({ items: [] as string[], input: "" }, "items", "input");
