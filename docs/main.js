var p=void 0,L0=0,C0=0,v=void 0;function Z0(f){if(p){if(!f.subs.includes(p))f.subs.push(p);if(!p.deps.includes(f))p.deps.push(f)}}function H0(f){f.version=++L0;let F=[...f.subs];for(let z of F)if(C0>0){if(!v)v=[];if(!v.includes(z))v.push(z)}else z.execute()}function J0(f){for(let F=0;F<f.deps.length;F++){let z=f.deps[F],D=z.subs.indexOf(f);if(D>-1)z.subs.splice(D,1)}f.deps.length=0}function P(f){let F=!0,z={execute(){if(!F)return;J0(z);let D=p;p=z;try{f()}finally{p=D}},deps:[],cleanup(){F=!1,J0(z)}};return z.execute(),()=>z.cleanup()}function P0(f,F=!1){let z={version:L0,subs:[]},D=f,G=function(J){if(arguments.length>0){if(F)throw TypeError("Cannot update a read-only signal.");if(!Object.is(D,J))D=J,H0(z);return}return Z0(z),D};return Object.defineProperty(G,"value",{get(){return Z0(z),D},set(J){if(F)throw TypeError("Cannot update a read-only signal.");if(!Object.is(D,J))D=J,H0(z)}}),G[B0]=!0,G.peek=()=>D,G}function Lf(f){let F={version:L0,subs:[]},z,D=-1,G={execute(){F.version=++L0;let L=[...F.subs];for(let s of L)if(C0>0){if(!v)v=[];if(!v.includes(s))v.push(s)}else s.execute()},deps:[],cleanup(){J0(G)}},J=()=>{J0(G);let L=p;p=G;try{let s=f();if(!Object.is(z,s))z=s;D=F.version}finally{p=L}};J();let W=function(){if(D!==F.version)J();return Z0(F),z};return Object.defineProperty(W,"value",{get(){return W()}}),W[B0]=!0,W.peek=()=>{if(D!==F.version)J();return z},W}var _0=Symbol("mutable");function V(f){return{value:f,[_0]:!0}}function k(f){return f!==null&&typeof f==="object"&&f[_0]===!0}var $0=Symbol("reactive-proxy"),K0=Symbol("raw");function O0(f){return f!==null&&typeof f==="object"&&$0 in f}function r(f,F=!1){let z=new Map,D=new WeakMap;function G(L){let s=z.get(L);if(!s)s={version:0,subs:[]},z.set(L,s);return s}if(F===!1||F instanceof Set&&F.size===0){let L=!1;for(let s in f)if(k(f[s])){L=!0;break}if(Array.isArray(f)&&!L){for(let s=0;s<f.length;s++)if(k(f[s])){L=!0;break}}if(!L){if(Array.isArray(f))for(let s=0;s<f.length;s++){let Y=f[s];if(Y&&typeof Y==="object"&&!k(Y)&&!O0(Y))f[s]=r(Y,!1)}else for(let s in f){let Y=f[s];if(Y&&typeof Y==="object"&&!k(Y)&&!O0(Y))f[s]=r(Y,!1)}Object.freeze(f)}}return new Proxy(f,{get(L,s,Y){if(s===K0||s==="$raw")return L;if(s===$0)return!0;let w=G(s);Z0(w);let q=Reflect.get(L,s,Y);if(k(q)){let X=D.get(q);if(X?.mutable)return X.mutable;let O=q.value;if(O!==null&&typeof O==="object"){let U=r(O,!0);if(!X)X={},D.set(q,X);return X.mutable=U,U}return O}if(q!==null&&typeof q==="object"&&!O0(q)){let X=typeof F==="boolean"&&F||F instanceof Set&&F.has(s),O=D.get(q);if(O){let Q=X?O.mutable:O.readonly;if(Q)return Q}let U=r(q,X);if(!O)O={},D.set(q,O);if(X)O.mutable=U;else O.readonly=U;return U}return q},set(L,s,Y,w){let q=typeof F==="boolean"&&F||F instanceof Set&&F.has(s),X=Reflect.get(L,s,w);if(!q&&k(X)){if(X.value===null||typeof X.value!=="object"){if(F===!1)F=new Set;if(F instanceof Set)F.add(s);q=!0}}if(!q)return!1;let O=Y!==null&&typeof Y==="object"&&K0 in Y?Y[K0]:Y,U=Array.isArray(L)&&s==="length";if(Object.is(X,O)&&!U)return!0;if(Reflect.set(L,s,O,w),X!==null&&typeof X==="object")D.delete(X);let Q=z.get(s);if(Q)H0(Q);return!0},has(L,s){if(s===$0||s===K0||s==="$raw")return!0;return Reflect.has(L,s)},ownKeys(L){return Reflect.ownKeys(L)},getOwnPropertyDescriptor(L,s){return Reflect.getOwnPropertyDescriptor(L,s)},deleteProperty(L,s){let Y=typeof F==="boolean"&&F||F instanceof Set&&F.has(s);if(!Y){let X=Reflect.get(L,s);if(k(X)){if(X.value===null||typeof X.value!=="object"){if(F===!1)F=new Set;if(F instanceof Set)F.add(s);Y=!0}}}if(!Y)return!1;let w=Reflect.has(L,s),q=Reflect.deleteProperty(L,s);if(w&&q){let X=z.get(s);if(X)H0(X)}return q}})}function A(f,...F){if(typeof f==="function")return Lf(f);if(f!==null&&typeof f==="object"&&!k(f))return r(f,new Set(F));if(k(f)){if(typeof f.value==="object"&&f.value!==null)return r(f.value,!0);return P0(f.value,!1)}return P0(f,!0)}var B0=Symbol("signal");function h(f){return typeof f==="function"&&f[B0]===!0}var A0=[];function x(f){A0.push(f)}function E(){A0.pop()}function R(){return A0[A0.length-1]??document.body}var U0=new WeakMap,Zf=0;function Hf(f,F,z){if(z)return z(f,F);if(typeof f==="object"&&f!==null){if(!U0.has(f))U0.set(f,Zf++);return`_o:${U0.get(f)}`}return`${typeof f}:${f}`}function W0(f,F,z){let D=document.createComment("Each");R().appendChild(D);let G=[],J=new Map;P(()=>{let W=typeof f==="function"&&!Array.isArray(f)?f():f,L=[],s=new Map,Y=new Set;for(let q=0;q<W.length;q++){let X=W[q],O=Hf(X,q,z);if(console.log({item:X,key:O}),Y.has(O))console.warn(`[Each] Duplicate key: "${O}". Keys must be unique.`);Y.add(O);let U=J.get(O);if(U&&(!z||U.item===X))L.push(U),s.set(O,U);else{let Q=document.createDocumentFragment();x(Q);try{F(X,q)}finally{E()}let z0=Array.from(Q.childNodes),M0={key:O,item:X,nodes:z0};if(L.push(M0),s.set(O,M0),U)for(let o of U.nodes)o.parentNode?.removeChild(o)}}for(let q of G)if(!s.has(q.key))for(let X of q.nodes)X.parentNode?.removeChild(X);let w=D;for(let q of L){let X=q.nodes[0];if(!X)continue;if(w.nextSibling!==X){let U=D.parentNode;if(!U)continue;for(let Q of q.nodes)U.insertBefore(Q,w.nextSibling)}w=q.nodes[q.nodes.length-1]||w}G=L,J.clear();for(let[q,X]of s)J.set(q,X)})}var Jf=new Set(["click","dblclick","input","change","keydown","keyup","keypress","mousedown","mouseup","mouseover","mouseout","mousemove","touchstart","touchend","touchmove","focusin","focusout","submit"]),q0=new WeakMap,p0=new Set;function Af(f){let{target:F,type:z}=f;while(F){let D=q0.get(F);if(D&&D[z]){if(Object.defineProperty(f,"currentTarget",{configurable:!0,value:F}),D[z](f),f.cancelBubble)break}F=F.parentElement}}function X0(f,F,z){if(Jf.has(F)){if(!p0.has(F))document.addEventListener(F,Af,{capture:!1,passive:!1}),p0.add(F);let D=q0.get(f);if(!D)D={},q0.set(f,D);D[F]=z}else f.addEventListener(F,z)}if(typeof window<"u")window.__eventHandlerMap=q0;function n(f){return(F,z)=>{let D=document.createElement(f),G,J;if(F===void 0);else if(y(F))J=F;else if(m(F)){if(G=F,z!==void 0)J=z}if(G)d(D,G);let W=[],L=(s)=>W.push(s);if(J){let s=document.createDocumentFragment();x(s);try{J(D,L)}finally{E()}D.appendChild(s)}if(R().appendChild(D),W.length>0)requestAnimationFrame(()=>{for(let s of W)s()});return D}}function T(f){return(F,z,D)=>{let G=document.createElement(f),J,W,L;if(F===void 0);else if(t(F)){if(J=F,z===void 0);else if(y(z))L=z;else if(m(z)){if(W=z,D!==void 0)L=D}}else if(y(F))L=F;else if(m(F)){if(W=F,z!==void 0&&y(z))L=z}if(J!==void 0)l(G,J);if(W)d(G,W);let s=[],Y=(w)=>s.push(w);if(L){let w=document.createDocumentFragment();x(w);try{L(G,Y)}finally{E()}G.appendChild(w)}if(R().appendChild(G),s.length>0)requestAnimationFrame(()=>{for(let w of s)w()});return G}}function h0(f){return(F,z,D)=>{let G=document.createElement(f),J,W,L,s;if(F===void 0);else if(t(F)){if(J=F,z===void 0);else if(x0(z))W=z;else if(y(z))s=z;else if(m(z)){if(L=z,D!==void 0)s=D}}else if(y(F))s=F;else if(m(F)){if(L=F,z!==void 0&&y(z))s=z}if(J!==void 0)l(G,J);if(W)X0(G,"click",W);if(L)d(G,L);let Y=[],w=(q)=>Y.push(q);if(s){let q=document.createDocumentFragment();x(q);try{s(G,w)}finally{E()}G.appendChild(q)}if(R().appendChild(G),Y.length>0)requestAnimationFrame(()=>{for(let q of Y)q()});return G}}function E0(){return(f,F,z)=>{let D=document.createElement("img"),G,J,W;if(f===void 0);else if(typeof f==="string"&&d0(f)){if(G=f,F===void 0);else if(typeof F==="string"){if(J=F,z!==void 0)W=z}else if(m(F))W=F}else if(m(f))W=f;if(G!==void 0)D.src=G;if(J!==void 0)D.alt=J;if(W)d(D,W);return R().appendChild(D),D}}function k0(){return(f,F,z)=>{let D=document.createElement("a"),G,J,W,L;if(f===void 0);else if(typeof f==="string"&&S0(f)){if(G=f,F===void 0);else if(t(F)){if(J=F,z!==void 0)W=z}else if(m(F))W=F}else if(y(f))L=f;else if(m(f)){if(W=f,F!==void 0&&y(F))L=F}if(G!==void 0)D.href=G;if(J!==void 0)l(D,J);if(W)d(D,W);let s=[],Y=(w)=>s.push(w);if(L){let w=document.createDocumentFragment();x(w);try{L(D,Y)}finally{E()}D.appendChild(w)}if(R().appendChild(D),s.length>0)requestAnimationFrame(()=>{for(let w of s)w()});return D}}function t(f){return typeof f==="string"||typeof f==="number"||h(f)&&(typeof f.peek()==="string"||typeof f.peek()==="number")}function x0(f){if(typeof f!=="function")return!1;if(h(f))return!1;return f.length<=1}function S0(f){if(typeof f!=="string")return!1;return/^(\/|#|https?:|mailto:|tel:|\.\.?\/)/.test(f)}function d0(f){if(typeof f!=="string")return!1;return/^(\/|https?:|data:image\/|blob:|\.\.?\/)/.test(f)||/\.(jpg|jpeg|png|gif|svg|webp|avif|ico|bmp)$/i.test(f)}var R0={className:"class",htmlFor:"for",httpEquiv:"http-equiv",acceptCharset:"accept-charset",accessKey:"accesskey",autoCapitalize:"autocapitalize",autoComplete:"autocomplete",autoFocus:"autofocus",autoPlay:"autoplay",colSpan:"colspan",contentEditable:"contenteditable",crossOrigin:"crossorigin",dateTime:"datetime",defaultChecked:"checked",defaultValue:"value",encType:"enctype",enterKeyHint:"enterkeyhint",fetchPriority:"fetchpriority",formAction:"formaction",formEnctype:"formenctype",formMethod:"formmethod",formNoValidate:"formnovalidate",formTarget:"formtarget",hrefLang:"hreflang",inputMode:"inputmode",isMap:"ismap",maxLength:"maxlength",minLength:"minlength",noModule:"nomodule",noValidate:"novalidate",playsInline:"playsinline",readOnly:"readonly",referrerPolicy:"referrerpolicy",rowSpan:"rowspan",srcDoc:"srcdoc",srcLang:"srclang",srcSet:"srcset",tabIndex:"tabindex",useMap:"usemap",itemScope:"itemscope",itemType:"itemtype",itemId:"itemid",itemProp:"itemprop",itemRef:"itemref",popoverTarget:"popovertarget",popoverTargetAction:"popovertargetaction",shadowRootMode:"shadowrootmode",shadowRootDelegatesFocus:"shadowrootdelegatesfocus",shadowRootClonable:"shadowrootclonable",shadowRootSerializable:"shadowrootserializable",controlsList:"controlslist",disablePictureInPicture:"disablepictureinpicture",disableRemotePlayback:"disableremoteplayback",allowFullscreen:"allowfullscreen",attributionSrc:"attributionsrc",elementTiming:"elementtiming"};function Wf(f){return["value","checked","selected","muted","currentTime","volume","indeterminate","defaultValue","defaultChecked","textContent","innerText"].includes(f)}function qf(f,F,z){switch(F){case"value":if("value"in f)f.value=String(z??"");break;case"checked":if("checked"in f)f.checked=Boolean(z);break;case"selected":if("selected"in f)f.selected=Boolean(z);break;case"muted":if("muted"in f)f.muted=Boolean(z);break;case"currentTime":if("currentTime"in f)f.currentTime=Number(z??0);break;case"volume":if("volume"in f)f.volume=Number(z??1);break;case"indeterminate":if("indeterminate"in f)f.indeterminate=Boolean(z);break;case"defaultValue":if("defaultValue"in f)f.defaultValue=String(z??"");break;case"defaultChecked":if("defaultChecked"in f)f.defaultChecked=Boolean(z);break;case"textContent":f.textContent=String(z??"");break;case"innerText":f.innerText=String(z??"");break}}function b0(f,F,z){if(F==="class"||F==="className"||F==="classList")Xf(f,z);else if(F==="style")Qf(f,z);else if(Wf(F))qf(f,F,z);else if(typeof z==="boolean")if(z)f.setAttribute(R0[F]??F,"");else f.removeAttribute(R0[F]??F);else f.setAttribute(R0[F]??F,String(z))}function d(f,F){for(let z in F){let D=F[z];if(D===null||D===void 0)continue;if(z.startsWith("on")&&typeof D==="function"){let G=z.slice(2).toLowerCase();X0(f,G,D)}else if(h(D))P(()=>b0(f,z,D.value));else b0(f,z,D)}}function Xf(f,F){if(typeof F==="string")f.className=F;else if(Array.isArray(F))f.className=F.filter(Boolean).join(" ");else if(typeof F==="object"&&F!==null){let z=!1;for(let G in F)if(h(F[G])){z=!0;break}let D=()=>{let G=[];for(let J in F){let W=F[J];if(h(W)?W.value:W)G.push(J)}f.className=G.join(" ")};if(z)P(D);else D()}}function Yf(f){return typeof f==="object"&&f!==null&&"type"in f&&typeof f.type==="string"}function N0(f){switch(f.type){case"rgb":return f.a!==void 0?`rgba(${f.r}, ${f.g}, ${f.b}, ${f.a})`:`rgb(${f.r}, ${f.g}, ${f.b})`;case"hsl":return f.a!==void 0?`hsla(${f.h}, ${f.s}%, ${f.l}%, ${f.a})`:`hsl(${f.h}, ${f.s}%, ${f.l}%)`;case"hwb":return f.a!==void 0?`hwb(${f.h} ${f.w}% ${f.b}% / ${f.a})`:`hwb(${f.h} ${f.w}% ${f.b}%)`;case"oklch":return f.a!==void 0?`oklch(${f.l}% ${f.c} ${f.h} / ${f.a})`:`oklch(${f.l}% ${f.c} ${f.h})`;case"lab":return f.alpha!==void 0?`lab(${f.l}% ${f.a} ${f.b} / ${f.alpha})`:`lab(${f.l}% ${f.a} ${f.b})`;case"lch":return f.alpha!==void 0?`lch(${f.l}% ${f.c} ${f.h} / ${f.alpha})`:`lch(${f.l}% ${f.c} ${f.h})`;case"oklab":return f.alpha!==void 0?`oklab(${f.l}% ${f.a} ${f.b} / ${f.alpha})`:`oklab(${f.l}% ${f.a} ${f.b})`;case"hex":return f.value;case"color":{let F=f.components.join(" ");return f.alpha!==void 0?`color(${f.space} ${F} / ${f.alpha})`:`color(${f.space} ${F})`}case"color-mix":{let F=typeof f.color1==="object"?N0(f.color1):f.color1,z=typeof f.color2==="object"?N0(f.color2):f.color2,D=f.percentage1!==void 0?`${f.percentage1}%`:"",G=f.percentage2!==void 0?`${f.percentage2}%`:"";return`color-mix(${f.method}, ${F} ${D}, ${z} ${G})`}}}function i0(f){if(f===null||f===void 0)return"";if(Yf(f))return N0(f);return String(f)}function v0(f,F,z){if(F.startsWith("--")){f.setProperty(F,z);return}if(F.startsWith("webkit")||F.startsWith("moz")||F.startsWith("ms")||F.startsWith("o")){let D=F.replace(/([A-Z])/g,"-$1").toLowerCase();f.setProperty(D,z);return}try{f[F]=z}catch{f.setProperty(F,z)}}function Qf(f,F){if(typeof F==="string")f.setAttribute("style",F);else if(typeof F==="object"&&F!==null){let z=!1;for(let D in F)if(h(F[D])){z=!0;break}if(z)P(()=>{for(let D in F){let G=F[D],J=h(G)?G.value:G;v0(f.style,D,i0(J))}});else for(let D in F){let G=F[D];v0(f.style,D,i0(G))}}}function l(f,F){if(h(F))P(()=>{f.textContent=String(F.value)});else f.textContent=String(F)}function m(f){return typeof f==="object"&&f!==null&&!h(f)&&!Array.isArray(f)}function y(f){return typeof f==="function"&&!h(f)}var C=k0(),V0=E0(),u=h0("button");var u0=T("h1"),n0=T("h2"),s0=T("h3"),G0=T("h4");var a=T("p"),Z=T("div");var Y0=T("section");var c0=T("header"),o0=T("footer");var Q0=T("pre");var I=T("span");var B=T("td"),S=T("th"),b=T("li");var e=n("ul");var T0=n("table"),m0=n("tbody"),y0=n("thead");var i=n("tr");var r0=n("nav");var jf=()=>{if(typeof localStorage<"u"&&localStorage.getItem("fia-theme"))return localStorage.getItem("fia-theme");if(typeof window<"u"&&window.matchMedia("(prefers-color-scheme: dark)").matches)return"dark";return"dark"},c=A(V({current:jf()}));P(()=>{let f=c.current;if(typeof localStorage<"u")localStorage.setItem("fia-theme",f);if(typeof document<"u"){if(document.documentElement.setAttribute("data-theme",f),!document.getElementById("fia-theme-styles")){let F=document.createElement("style");F.id="fia-theme-styles",F.textContent=`
                :root {
