...
div({ class: "app" }, () => {
  ...
  button({ textContent: "-", onclick: () => state.count-- });
});`)}),K("Mounting",()=>{I("For Single Page Apps (SPAs), use the mount helper to attach to a root element."),Z(`import { mount, div } from "fia";

const App = () => div(() => {
  // Your app structure
});

mount(App, "#app"); // Clears #app and mounts App`)})}),N("Element API","element-api",()=>{I("Fia elements have a simple, consistent API. Functions match HTML tag names."),Z(`el()                      // Empty element
el({ props })             // Props only
el(() => { ... })         // Children only
...
});`)})}),N("Reactivity","reactivity",()=>{K("Signals",()=>{I("Signals are the primitive units of reactivity."),Z(`const count = $(0);
console.log(count.value); // 0
count.value++;`)}),K("Reactive Stores",()=>{I("Fia stores are immutable by default for predictability."),Z(`// Immutable Store
const config = $({ theme: "dark" });
// config.theme = "light"; // Error!

// Mutable Store (Opt-in)
const state = $({ count: 0 }, "count");
state.count++; // Works!`),g0("Destructuring breaks reactivity. Always access properties directly: state.count","warning")}),K("Computed Values",()=>{I("Computed signals automatically track dependencies and update when they change."),Z(`const count = $(0);
const doubled = $(() => count.value * 2);`)}),K("Effects",()=>{I("Use $e() to run side effects when dependencies change."),Z(`$e(() => {
  console.log("Count changed to:", count.value);
});`)})}),N("Immutability","immutability",()=>{I("Fia embraces an Immutable-by-Default philosophy for state management. This differs from many other signals-based frameworks but aligns with functional programming principles."),K("Working with Immutable State",()=>{I("When a store is immutable, you update state by replacing objects, not mutating properties."),Z(`const state = $({ 
  user: { name: "Evan", score: 10 } 
}, "user"); // 'user' key is mutable

// Correct: Replace the object
state.user = { ...state.user, score: state.user.score + 1 };`)})}),N("Control Flow","control-flow",()=>{K("Show",()=>{I("Conditionally render content that updates when the condition changes."),Z('Show(() => isVisible.value, () => div({ textContent: "Hello!" }));')}),K("Each",()=>{I("Reactive list rendering that re-renders efficiently."),Z(`const items = $({ list: ["Apple", "Banana"] });
Each(() => items.list, item => li({ textContent: item }));`)}),K("Match",()=>{I("Reactive pattern matching for switch/case logic."),Z(`Match(() => status.value, {
  loading: () => p({ textContent: "Loading..." }),
  success: () => div({ textContent: "Data loaded!" }),
  _: () => p({ textContent: "Unknown state" }),
});`)})}),N("Component Composition","components",()=>{I("In Fia, components are just functions. There is no special class or type."),K("Basic Component",()=>{Z(`function Button(props: { text: string }) {
  return button({
    textContent: props.text,
    class: "btn-primary"
  });
}`)}),K("Children & Layouts",()=>{I("To create wrapper components, pass a callback function as a child prop."),Z(`function Card(props, children) {
  return div({ class: "card" }, () => {
    children();
  });
}`)})}),N("SVG","svg",()=>{I("Fia supports SVG elements with full type safety."),Z(`import { svg, svgCircle } from "fia";

svg({ width: 100, height: 100 }, () => {
  svgCircle({ cx: 50, cy: 50, r: 40, fill: "red" });
});`)}),N("Performance","performance",()=>{Y0(["Event Delegation: Single listener per event type.","Automatic Batching: DOM updates are batched via standard event loop microtasks.","Fragment Insertion: Children are collected in DocumentFragments before insertion."])}),N("Examples","examples",()=>{K("\uD83D\uDFE2 Beginner",()=>{D("1. Hello World",()=>{I("The simplest possible Fia code."),Z('h1({ textContent: "Hello, World!" });')}),D("2. Counter",()=>{I("Signals hold reactive state."),Z(`const count = $(0);
...
ul(() => items.forEach(item => li({ textContent: item })));`)})}),K("\uD83D\uDFE1 Intermediate",()=>{D("6. Reactive Store Counter",()=>{I("Objects passed to $() become reactive stores."),Z(`const state = $({ count: 0 }, "count");
...
});`)})}),K("\uD83D\uDD34 Advanced",()=>{D("11. Todo App",()=>{I("A complete todo app using Each."),Z(`const todos = $({ items: [] as string[], input: "" }, "items", "input");
...
});`)})})})});var b0=()=>f({id:"landing-page"},()=>{D0(),G0(),K0(),M0(),L0(),X0()});b0();
