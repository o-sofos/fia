var _=void 0,L0=0,n0=0,c=void 0;function q0(f){if(_){if(!f.subs.includes(_))f.subs.push(_);if(!_.deps.includes(f))_.deps.push(f)}}function Q0(f){f.version=++L0;let F=[...f.subs];for(let z of F)if(n0>0){if(!c)c=[];if(!c.includes(z))c.push(z)}else z.execute()}function W0(f){for(let F=0;F<f.deps.length;F++){let z=f.deps[F],D=z.subs.indexOf(f);if(D>-1)z.subs.splice(D,1)}f.deps.length=0}function y(f){let F=!0,z={execute(){if(!F)return;W0(z);let D=_;_=z;try{f()}finally{_=D}},deps:[],cleanup(){F=!1,W0(z)}};return z.execute(),()=>z.cleanup()}function u0(f,F=!1){let z={version:L0,subs:[]},D=f,Z=function(L){if(arguments.length>0){if(F)throw TypeError("Cannot update a read-only signal.");if(!Object.is(D,L))D=L,Q0(z);return}return q0(z),D};return Object.defineProperty(Z,"value",{get(){return q0(z),D},set(L){if(F)throw TypeError("Cannot update a read-only signal.");if(!Object.is(D,L))D=L,Q0(z)}}),Z[m0]=!0,Z.peek=()=>D,Z}function wf(f){let F={version:L0,subs:[]},z,D=-1,Z={execute(){F.version=++L0;let G=[...F.subs];for(let K of G)if(n0>0){if(!c)c=[];if(!c.includes(K))c.push(K)}else K.execute()},deps:[],cleanup(){W0(Z)}},L=()=>{W0(Z);let G=_;_=Z;try{let K=f();if(!Object.is(z,K))z=K;D=F.version}finally{_=G}};L();let Q=function(){if(D!==F.version)L();return q0(F),z};return Object.defineProperty(Q,"value",{get(){return Q()}}),Q[m0]=!0,Q.peek=()=>{if(D!==F.version)L();return z},Q}var c0=Symbol("mutable");function p(f){return{value:f,[c0]:!0}}function b(f){return f!==null&&typeof f==="object"&&f[c0]===!0}var T0=Symbol("reactive-proxy"),J0=Symbol("raw");function R0(f){return f!==null&&typeof f==="object"&&T0 in f}function t(f,F=!1){let z=new Map,D=new WeakMap;function Z(G){let K=z.get(G);if(!K)K={version:0,subs:[]},z.set(G,K);return K}if(F===!1||F instanceof Set&&F.size===0){let G=!1;for(let K in f)if(b(f[K])){G=!0;break}if(Array.isArray(f)&&!G){for(let K=0;K<f.length;K++)if(b(f[K])){G=!0;break}}if(!G){if(Array.isArray(f))for(let K=0;K<f.length;K++){let Y=f[K];if(Y&&typeof Y==="object"&&!b(Y)&&!R0(Y))f[K]=t(Y,!1)}else for(let K in f){let Y=f[K];if(Y&&typeof Y==="object"&&!b(Y)&&!R0(Y))f[K]=t(Y,!1)}Object.freeze(f)}}return new Proxy(f,{get(G,K,Y){if(K===J0||K==="$raw")return G;if(K===T0)return!0;let M=Z(K);q0(M);let W=Reflect.get(G,K,Y);if(b(W)){let X=D.get(W);if(X?.mutable)return X.mutable;let w=W.value;if(w!==null&&typeof w==="object"){let B=t(w,!0);if(!X)X={},D.set(W,X);return X.mutable=B,B}return w}if(W!==null&&typeof W==="object"&&!R0(W)){let X=typeof F==="boolean"&&F||F instanceof Set&&F.has(K),w=D.get(W);if(w){let A=X?w.mutable:w.readonly;if(A)return A}let B=t(W,X);if(!w)w={},D.set(W,w);if(X)w.mutable=B;else w.readonly=B;return B}return W},set(G,K,Y,M){let W=typeof F==="boolean"&&F||F instanceof Set&&F.has(K),X=Reflect.get(G,K,M);if(!W&&b(X)){if(X.value===null||typeof X.value!=="object"){if(F===!1)F=new Set;if(F instanceof Set)F.add(K);W=!0}}if(!W)return!1;let w=Y!==null&&typeof Y==="object"&&J0 in Y?Y[J0]:Y,B=Array.isArray(G)&&K==="length";if(Object.is(X,w)&&!B)return!0;if(Reflect.set(G,K,w,M),X!==null&&typeof X==="object")D.delete(X);let A=z.get(K);if(A)Q0(A);return!0},has(G,K){if(K===T0||K===J0||K==="$raw")return!0;return Reflect.has(G,K)},ownKeys(G){return Reflect.ownKeys(G)},getOwnPropertyDescriptor(G,K){return Reflect.getOwnPropertyDescriptor(G,K)},deleteProperty(G,K){let Y=typeof F==="boolean"&&F||F instanceof Set&&F.has(K);if(!Y){let X=Reflect.get(G,K);if(b(X)){if(X.value===null||typeof X.value!=="object"){if(F===!1)F=new Set;if(F instanceof Set)F.add(K);Y=!0}}}if(!Y)return!1;let M=Reflect.has(G,K),W=Reflect.deleteProperty(G,K);if(M&&W){let X=z.get(K);if(X)Q0(X)}return W}})}function j(f,...F){if(typeof f==="function")return wf(f);if(f!==null&&typeof f==="object"&&!b(f))return t(f,new Set(F));if(b(f)){if(typeof f.value==="object"&&f.value!==null)return t(f.value,!0);return u0(f.value,!1)}return u0(f,!0)}var m0=Symbol("signal");function E(f){return typeof f==="function"&&f[m0]===!0}var X0=[];function h(f){X0.push(f)}function k(){X0.pop()}function V(){return X0[X0.length-1]??document.body}var P0=new WeakMap,Bf=0;function $f(f,F,z){if(z)return z(f,F);if(typeof f==="object"&&f!==null){if(!P0.has(f))P0.set(f,Bf++);return`_o:${P0.get(f)}`}return`${typeof f}:${f}`}function j0(f,F,z){let D=document.createComment("Each");V().appendChild(D);let Z=[],L=new Map;y(()=>{let Q=typeof f==="function"&&!Array.isArray(f)?f():f,G=[],K=new Map,Y=new Set;for(let W=0;W<Q.length;W++){let X=Q[W],w=$f(X,W,z);if(console.log({item:X,key:w}),Y.has(w))console.warn(`[Each] Duplicate key: "${w}". Keys must be unique.`);Y.add(w);let B=L.get(w);if(B&&(!z||B.item===X))G.push(B),K.set(w,B);else{let A=document.createDocumentFragment();h(A);try{F(X,W)}finally{k()}let S=Array.from(A.childNodes),K0={key:w,item:X,nodes:S};if(G.push(K0),K.set(w,K0),B)for(let n of B.nodes)n.parentNode?.removeChild(n)}}for(let W of Z)if(!K.has(W.key))for(let X of W.nodes)X.parentNode?.removeChild(X);let M=D;for(let W of G){let X=W.nodes[0];if(!X)continue;if(M.nextSibling!==X){let B=D.parentNode;if(!B)continue;for(let A of W.nodes)B.insertBefore(A,M.nextSibling)}M=W.nodes[W.nodes.length-1]||M}Z=G,L.clear();for(let[W,X]of K)L.set(W,X)})}var Uf=new Set(["click","dblclick","input","change","keydown","keyup","keypress","mousedown","mouseup","mouseover","mouseout","mousemove","touchstart","touchend","touchmove","focusin","focusout","submit"]),Y0=new WeakMap,o0=new Set;function Vf(f){let{target:F,type:z}=f;while(F){let D=Y0.get(F);if(D&&D[z]){if(Object.defineProperty(f,"currentTarget",{configurable:!0,value:F}),D[z](f),f.cancelBubble)break}F=F.parentElement}}function A0(f,F,z){if(Uf.has(F)){if(!o0.has(F))document.addEventListener(F,Vf,{capture:!1,passive:!1}),o0.add(F);let D=Y0.get(f);if(!D)D={},Y0.set(f,D);D[F]=z}else f.addEventListener(F,z)}if(typeof window<"u")window.__eventHandlerMap=Y0;function o(f){return(F,z)=>{let D=document.createElement(f),Z,L;if(F===void 0);else if(m(F))L=F;else if(T(F)){if(Z=F,z!==void 0)L=z}if(Z)x(D,Z);let Q=[],G=(K)=>Q.push(K);if(L){let K=document.createDocumentFragment();h(K);try{L(D,G)}finally{k()}D.appendChild(K)}if(V().appendChild(D),Q.length>0)requestAnimationFrame(()=>{for(let K of Q)K()});return D}}function R(f){return(F,z,D)=>{let Z=document.createElement(f),L,Q,G;if(F===void 0);else if(a(F)){if(L=F,z===void 0);else if(m(z))G=z;else if(T(z)){if(Q=z,D!==void 0)G=D}}else if(m(F))G=F;else if(T(F)){if(Q=F,z!==void 0&&m(z))G=z}if(L!==void 0)e(Z,L);if(Q)x(Z,Q);let K=[],Y=(M)=>K.push(M);if(G){let M=document.createDocumentFragment();h(M);try{G(Z,Y)}finally{k()}Z.appendChild(M)}if(V().appendChild(Z),K.length>0)requestAnimationFrame(()=>{for(let M of K)M()});return Z}}function r0(f){return(F,z,D)=>{let Z=document.createElement(f),L,Q,G,K;if(F===void 0);else if(a(F)){if(L=F,z===void 0);else if(l0(z))Q=z;else if(m(z))K=z;else if(T(z)){if(G=z,D!==void 0)K=D}}else if(m(F))K=F;else if(T(F)){if(G=F,z!==void 0&&m(z))K=z}if(L!==void 0)e(Z,L);if(Q)A0(Z,"click",Q);if(G)x(Z,G);let Y=[],M=(W)=>Y.push(W);if(K){let W=document.createDocumentFragment();h(W);try{K(Z,M)}finally{k()}Z.appendChild(W)}if(V().appendChild(Z),Y.length>0)requestAnimationFrame(()=>{for(let W of Y)W()});return Z}}function t0(){return(f,F,z)=>{let D=document.createElement("img"),Z,L,Q;if(f===void 0);else if(typeof f==="string"&&a0(f)){if(Z=f,F===void 0);else if(typeof F==="string"){if(L=F,z!==void 0)Q=z}else if(T(F))Q=F}else if(T(f))Q=f;if(Z!==void 0)D.src=Z;if(L!==void 0)D.alt=L;if(Q)x(D,Q);return V().appendChild(D),D}}function e0(){return(f,F,z)=>{let D=document.createElement("a"),Z,L,Q,G;if(f===void 0);else if(typeof f==="string"&&ff(f)){if(Z=f,F===void 0);else if(a(F)){if(L=F,z!==void 0)Q=z}else if(T(F))Q=F}else if(m(f))G=f;else if(T(f)){if(Q=f,F!==void 0&&m(F))G=F}if(Z!==void 0)D.href=Z;if(L!==void 0)e(D,L);if(Q)x(D,Q);let K=[],Y=(M)=>K.push(M);if(G){let M=document.createDocumentFragment();h(M);try{G(D,Y)}finally{k()}D.appendChild(M)}if(V().appendChild(D),K.length>0)requestAnimationFrame(()=>{for(let M of K)M()});return D}}function a(f){return typeof f==="string"||typeof f==="number"||E(f)&&(typeof f.peek()==="string"||typeof f.peek()==="number")}function l0(f){if(typeof f!=="function")return!1;if(E(f))return!1;return f.length<=1}function ff(f){if(typeof f!=="string")return!1;return/^(\/|#|https?:|mailto:|tel:|\.\.?\/)/.test(f)}function a0(f){if(typeof f!=="string")return!1;return/^(\/|https?:|data:image\/|blob:|\.\.?\/)/.test(f)||/\.(jpg|jpeg|png|gif|svg|webp|avif|ico|bmp)$/i.test(f)}var y0={className:"class",htmlFor:"for",httpEquiv:"http-equiv",acceptCharset:"accept-charset",accessKey:"accesskey",autoCapitalize:"autocapitalize",autoComplete:"autocomplete",autoFocus:"autofocus",autoPlay:"autoplay",colSpan:"colspan",contentEditable:"contenteditable",crossOrigin:"crossorigin",dateTime:"datetime",defaultChecked:"checked",defaultValue:"value",encType:"enctype",enterKeyHint:"enterkeyhint",fetchPriority:"fetchpriority",formAction:"formaction",formEnctype:"formenctype",formMethod:"formmethod",formNoValidate:"formnovalidate",formTarget:"formtarget",hrefLang:"hreflang",inputMode:"inputmode",isMap:"ismap",maxLength:"maxlength",minLength:"minlength",noModule:"nomodule",noValidate:"novalidate",playsInline:"playsinline",readOnly:"readonly",referrerPolicy:"referrerpolicy",rowSpan:"rowspan",srcDoc:"srcdoc",srcLang:"srclang",srcSet:"srcset",tabIndex:"tabindex",useMap:"usemap",itemScope:"itemscope",itemType:"itemtype",itemId:"itemid",itemProp:"itemprop",itemRef:"itemref",popoverTarget:"popovertarget",popoverTargetAction:"popovertargetaction",shadowRootMode:"shadowrootmode",shadowRootDelegatesFocus:"shadowrootdelegatesfocus",shadowRootClonable:"shadowrootclonable",shadowRootSerializable:"shadowrootserializable",controlsList:"controlslist",disablePictureInPicture:"disablepictureinpicture",disableRemotePlayback:"disableremoteplayback",allowFullscreen:"allowfullscreen",attributionSrc:"attributionsrc",elementTiming:"elementtiming"};function Nf(f){return["value","checked","selected","muted","currentTime","volume","indeterminate","defaultValue","defaultChecked","textContent","innerText"].includes(f)}function Rf(f,F,z){switch(F){case"value":if("value"in f)f.value=String(z??"");break;case"checked":if("checked"in f)f.checked=Boolean(z);break;case"selected":if("selected"in f)f.selected=Boolean(z);break;case"muted":if("muted"in f)f.muted=Boolean(z);break;case"currentTime":if("currentTime"in f)f.currentTime=Number(z??0);break;case"volume":if("volume"in f)f.volume=Number(z??1);break;case"indeterminate":if("indeterminate"in f)f.indeterminate=Boolean(z);break;case"defaultValue":if("defaultValue"in f)f.defaultValue=String(z??"");break;case"defaultChecked":if("defaultChecked"in f)f.defaultChecked=Boolean(z);break;case"textContent":f.textContent=String(z??"");break;case"innerText":f.innerText=String(z??"");break}}function Ff(f,F,z){if(F==="class"||F==="className"||F==="classList")Tf(f,z);else if(F==="style")Pf(f,z);else if(Nf(F))Rf(f,F,z);else if(typeof z==="boolean")if(z)f.setAttribute(y0[F]??F,"");else f.removeAttribute(y0[F]??F);else f.setAttribute(y0[F]??F,String(z))}function x(f,F){for(let z in F){let D=F[z];if(D===null||D===void 0)continue;if(z.startsWith("on")&&typeof D==="function"){let Z=z.slice(2).toLowerCase();A0(f,Z,D)}else if(E(D))y(()=>Ff(f,z,D.value));else Ff(f,z,D)}}function Tf(f,F){if(typeof F==="string")f.className=F;else if(Array.isArray(F))f.className=F.filter(Boolean).join(" ");else if(typeof F==="object"&&F!==null){let z=!1;for(let Z in F)if(E(F[Z])){z=!0;break}let D=()=>{let Z=[];for(let L in F){let Q=F[L];if(E(Q)?Q.value:Q)Z.push(L)}f.className=Z.join(" ")};if(z)y(D);else D()}}function mf(f){return typeof f==="object"&&f!==null&&"type"in f&&typeof f.type==="string"}function g0(f){switch(f.type){case"rgb":return f.a!==void 0?`rgba(${f.r}, ${f.g}, ${f.b}, ${f.a})`:`rgb(${f.r}, ${f.g}, ${f.b})`;case"hsl":return f.a!==void 0?`hsla(${f.h}, ${f.s}%, ${f.l}%, ${f.a})`:`hsl(${f.h}, ${f.s}%, ${f.l}%)`;case"hwb":return f.a!==void 0?`hwb(${f.h} ${f.w}% ${f.b}% / ${f.a})`:`hwb(${f.h} ${f.w}% ${f.b}%)`;case"oklch":return f.a!==void 0?`oklch(${f.l}% ${f.c} ${f.h} / ${f.a})`:`oklch(${f.l}% ${f.c} ${f.h})`;case"lab":return f.alpha!==void 0?`lab(${f.l}% ${f.a} ${f.b} / ${f.alpha})`:`lab(${f.l}% ${f.a} ${f.b})`;case"lch":return f.alpha!==void 0?`lch(${f.l}% ${f.c} ${f.h} / ${f.alpha})`:`lch(${f.l}% ${f.c} ${f.h})`;case"oklab":return f.alpha!==void 0?`oklab(${f.l}% ${f.a} ${f.b} / ${f.alpha})`:`oklab(${f.l}% ${f.a} ${f.b})`;case"hex":return f.value;case"color":{let F=f.components.join(" ");return f.alpha!==void 0?`color(${f.space} ${F} / ${f.alpha})`:`color(${f.space} ${F})`}case"color-mix":{let F=typeof f.color1==="object"?g0(f.color1):f.color1,z=typeof f.color2==="object"?g0(f.color2):f.color2,D=f.percentage1!==void 0?`${f.percentage1}%`:"",Z=f.percentage2!==void 0?`${f.percentage2}%`:"";return`color-mix(${f.method}, ${F} ${D}, ${z} ${Z})`}}}function zf(f){if(f===null||f===void 0)return"";if(mf(f))return g0(f);return String(f)}function Df(f,F,z){if(F.startsWith("--")){f.setProperty(F,z);return}if(F.startsWith("webkit")||F.startsWith("moz")||F.startsWith("ms")||F.startsWith("o")){let D=F.replace(/([A-Z])/g,"-$1").toLowerCase();f.setProperty(D,z);return}try{f[F]=z}catch{f.setProperty(F,z)}}function Pf(f,F){if(typeof F==="string")f.setAttribute("style",F);else if(typeof F==="object"&&F!==null){let z=!1;for(let D in F)if(E(F[D])){z=!0;break}if(z)y(()=>{for(let D in F){let Z=F[D],L=E(Z)?Z.value:Z;Df(f.style,D,zf(L))}});else for(let D in F){let Z=F[D];Df(f.style,D,zf(Z))}}}function e(f,F){if(E(F))y(()=>{f.textContent=String(F.value)});else f.textContent=String(F)}function T(f){return typeof f==="object"&&f!==null&&!E(f)&&!Array.isArray(f)}function m(f){return typeof f==="function"&&!E(f)}var g=e0(),C0=t0(),r=r0("button");var Kf=R("h1"),Zf=R("h2"),Z0=R("h3"),G0=R("h4");var f0=R("p"),J=R("div");var M0=R("section");var Gf=R("header"),Hf=R("footer");var s0=R("pre");var O=R("span");var U=R("td"),d=R("th"),v=R("li");var F0=o("ul");var _0=o("table"),p0=o("tbody"),E0=o("thead");var i=o("tr");var Jf=o("nav");var Lf=o("canvas");var yf=()=>{if(typeof localStorage<"u"&&localStorage.getItem("fia-theme"))return localStorage.getItem("fia-theme");if(typeof window<"u"&&window.matchMedia("(prefers-color-scheme: dark)").matches)return"dark";return"dark"},u=j(p({current:yf()}));y(()=>{let f=u.current;if(typeof localStorage<"u")localStorage.setItem("fia-theme",f);if(typeof document<"u"){if(document.documentElement.setAttribute("data-theme",f),!document.getElementById("fia-theme-styles")){let F=document.createElement("style");F.id="fia-theme-styles",F.textContent=`
                :root {
...
el({ props })             // Props only
el(() => { ... })         // Children only
el({ props }, () => { })  // Props + children`),s("Event Handlers",()=>{H("Event handlers are delegated automatically for performance."),q(`button("Click me", () => console.log("clicked!"));

input({
  type: "text",
  oninput: (e) => console.log(e.currentTarget.value),
});`)}),s("Nesting Elements",()=>{H("Use a callback function to nest elements."),q(`div({ class: "card" }, () => {
  h1("Title");
  p("Description");
});`)}),s("Void Elements",()=>{H("Elements like input, img, br only accept props."),q(`input({ type: "email", placeholder: "you@example.com" });
img("/photo.jpg", "Photo");
br();`)}),s("onMount Callback",()=>{H("Access layout properties after the element is in the DOM."),q(`div((el, onMount) => {
...
});`}]),N("Elements: article, section, nav, form, ul, ol, table, canvas, video, and more.")}),$("Text Elements",()=>{H("Optimized for elements that commonly hold text content. Click each tab to see different ways to create the same heading:"),w0([{label:"Empty",code:`// Empty element
...
});`}]),N("Elements: h1-h6, p, div, span, label, li, td, th, strong, em, code, and more.")}),$("Interactive Elements",()=>{H("Special factories for interactive elements with convenient text + click handler shorthand:"),w0([{label:"Text + Click ",code:`// Text + click handler shorthand
...
const score = $(Mut(0));
score.value = 10;`)}),s("2. Objects",()=>{H("Objects are shallowly immutable by default. You cannot add, remove, or change properties."),q(`const user = $({ name: "Evan", age: 30 });
...
// unless the parent key is also mutable.`)}),s("Secure Immutability by Design",()=>{H("Fia's reactive stores are designed to prevent accidental leaks of reactivity. When you spread a store, you get a plain object snapshot, not a reactive clone."),q(`const original = $({ name: "Evan", details: { age: 30 } });
const snapshot = { ...original };

// To create a truly independent reactive copy:
const clone = $({ ...original }); // New store with copied values`),N("This behavior ensures you never accidentally pass reactivity where a plain value was expected, maintaining explicit data flow.","info")}),s("3. Arrays",()=>{H("Arrays are immutable by default. Methods that mutate (push, pop, splice, sort) are typed to not exist or error."),q(`const list = $({ items: [1, 2, 3] });
...
todos.list.splice(0, 1);`)}),s("4. Nested Objects (Deep Reactivity)",()=>{H("Deeply nested objects inherit the mutability context of their parent property assignment, but by default, Fia encourages replacing nested objects."),q(`const app = $(Mut({
  settings: {
...
// ✅ State, focus, scroll position preserved`)}),s("Custom Key Function (Optional)",()=>{H("For explicit control (e.g., database IDs), provide a custom key function:"),q(`// Optional: use database ID as key
Each(
  () => todos.items,
  (todo) => {
    li(() => {
...
);`),N("How automatic keying works: Objects/arrays get stable internal IDs via WeakMap (no memory leaks). Primitives are keyed by type:value. Custom keyFn takes precedence when provided.","info")}),s("When to Use Custom Keys",()=>{H("Automatic keying works great in most cases, but provide a custom keyFn when:"),q(`// ✅ Automatic keying works:
...
// - State and scroll position preserved ✅`)}),s("Custom Key Function Best Practices",()=>{H("While automatic keying works great, you may want custom keys for specific use cases:"),q(`// ✅ Good: Database ID (explicit control)
(item) => item.id
...
Match(status, {
  ...
  _: () => span("Unknown")
});`)}),s("Booleans","match-booleans",()=>{H("Boolean values are automatically converted to string keys:"),q(`const isActive = $(Mut(true));

Match(isActive, {
  "true": () => "✅ Active",
  "false": () => "❌ Inactive"
});`)}),s("Numbers","match-numbers",()=>{H("Numbers support exact matching:"),q(`const count = $(Mut(2));
...
document.body
  └── 1 click handler (delegated)
      └── WeakMap<Element, Handler>`),s("How it works",()=>{z0(["One global listener per event type (click, input, etc.)","Handlers stored in WeakMap<Element, Handler>","Automatic cleanup when elements are removed","Dynamic elements work without rebinding"])}),s("Benefits",()=>{z0(["Memory efficient: 100 buttons = 1 listener (not 100)","Faster event dispatch: Single lookup","No memory leaks from forgotten listeners","Works with dynamically created elements"])}),q(`// Create 1,000 buttons - still only 1 click listener!
ul(() => {
...
// Single appendChild(fragment)`),s("How it works",()=>{z0(["Children callback creates a DocumentFragment","All child elements append to fragment (in-memory)","Complete fragment inserted in one operation","Browser performs one reflow instead of multiple"])}),s("Benefits",()=>{z0(["Single reflow: N insertions = 1 reflow (not N)","Faster rendering with 10+ children","Automatic - no manual optimization needed","Composable with nested structures"])}),q(`// Fia automatically batches 100 elements
div(() => {
  h1("Title");
  ul(() => {
    for (let i = 0; i < 100; i++) {
...
});`)}),$("Best Practices",()=>{s("1. Batch Multiple Updates",()=>{q(`import { batch } from "fia";

batch(() => {
  state.name = "Alice";
  state.age = 30;
  state.active = true;
}); // Triggers one effect run`)}),s("2. Use peek() for Non-Reactive Reads",()=>{q(`const count = $(Mut(0));
const threshold = $(10);
...
});`)}),s("3. Memoize Expensive Computations",()=>{q(`// Bad: Re-computes on every access
const doubled = count.value * 2;

// Good: Computed once, cached until count changes
const doubled = $(() => count.value * 2);`)})})}),C(()=>I.value.docs.examples.title,"examples",()=>{$("\uD83D\uDFE2 Beginner",()=>{s("1. Hello World",()=>{H("The simplest possible Fia code."),q('h1("Hello, World!");')}),s("2. Counter",()=>{H("Signals hold reactive state."),q(`const count = $(Mut(0));
button("+", () => count.value++);
p(count);`)}),s("3. Toggle",()=>{H("Computed signals derive values from other signals."),q(`const visible = $(Mut(true));
button("Toggle", () => visible.value = !visible.value);
Show(visible, () => p("Now you see me!"));`)}),s("4. Input Binding",()=>{H("Two-way binding is manual but explicit."),q('const name = $(Mut(""));\ninput({ type: "text", oninput: (e) => name.value = e.currentTarget.value });\np($(() => `Hello, ${name.value || "stranger"}!`));')}),s("5. List Rendering (Static)",()=>{H("For simple static lists, forEach works fine."),q(`const items = ["Apple", "Banana", "Cherry"];
ul(() => Each(items, li));`)})}),$("\uD83D\uDFE1 Intermediate",()=>{s("6. Reactive Store Counter",()=>{H("Objects passed to $() become reactive stores."),q(`const state = $(Mut({ count: 0 }));

div(() => {
  h1($(() => \`Count: \${state.count}\`));
  button("+", () => state.count++);
  button("-", () => state.count--);
});`)}),s("7. Conditional Classes",()=>{H("Computed signals work in class props too."),q(`const active = $(Mut(false));
button($(() => active.value ? "Deactivate" : "Activate"), () => active.value = !active.value);`)}),s("8. Form Handling",()=>{H("Reactive stores are perfect for forms."),q(`const formData = $(Mut({ email: "", password: "" }));
...
});`)}),s("9. Computed Values",()=>{H("Track dependencies automatically."),q('const state = $(Mut({ price: 100, quantity: 2 }));\nconst total = $(() => state.price * state.quantity);\n\ndiv(() => {\n  p($(() => `Price: $${state.price}`));\n  p($(() => `Qty: ${state.quantity}`));\n  p($(() => `Total: $${total.value}`));\n  button("Add", () => state.quantity++);\n});')}),s("10. Dynamic Styling",()=>{H("Reactive styles allow theming."),q(`const theme = $(Mut("light"));
...
});`)})}),$("\uD83D\uDD34 Advanced",()=>{s("11. Control Flow Combo (Each + Show + Match)",()=>{H("A complete task manager combining all control flow components:"),q(`// Task manager example combining Each, Show, and Match
...
});`)}),s("12. Todo App",()=>{H("A complete todo app using Each."),q(`const todos = $(Mut({ items: [] as string[], input: "" }));
...
});`)}),s("12. Tabs Component",()=>{H("Track active index and conditionally render."),q(`const tabs = ["Home", "About", "Contact"];
const active = $(Mut(0));
...
});`)}),s("13. Async Data Fetching",()=>{H("Use Match for loading states."),q(`const state = $(Mut({
...
});`)}),s("14. Modal Dialog",()=>{H("Modal patterns with explicit types."),q(`const modal = $(Mut({ open: false, title: "" }));
...
});`)})})})})});var Ef=()=>J({id:"landing-page"},()=>{jf(),Xf(),Yf(),Af(),Mf(),If(),sf()});Ef();
